...
class GameStateManager:
    ...
    public_game_list: dict[str, typing.Callable]  # Names of the Public Games currently available
    private_game_list: dict[str, PrivateGame]  # Names of the Private Games currently available.
    ...
    async def start_new_private_game(
        ...
        if not game_name or game_name not in self.private_game_list:
            ...
            return await inter.send(buf)
        if inter.channel_id != self.private_game_channel.id:
            return await inter.send(f"You can only play games in {self.private_game_channel.mention}.")
